...
const double Kp_turns_FWD    = 0.3;  // start moderate; tune 0.5–1.5 range

// --- Backward gains (used for turning one wheel backward) ---
const double K_RIGHT_PWM_BACK = 4.3;
const double K_LEFT_PWM_BACK  = 4.7;   // not used now, but kept if needed later

// --- Base forward speed per wheel (can be different if one wheel is stronger) ---
const double S_BASE_FWD_RIGHT = 1.06;
const double S_BASE_FWD_LEFT  = 0.99;

// --- Turn speed per wheel (can be different if turning is uneven) ---
const double S_TURN_RIGHT = 1.0;
const double S_TURN_LEFT  = 1.0;
...
void setup() {
  ...
  Serial.println("\nCommands:");
  Serial.println("  'q' - Drive a 40cm x 40cm square (4 sides)");
...
void moveForwardDistance(double distance_m) {
  ...
  Serial.println("\n--- FORWARD distance END -------upload without wifi---- ---");
...
void runSquare40cm() {
  ...
  // ➤ NEW: Final 90° turn to restore original orientation
